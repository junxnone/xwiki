...
### ✅ 开启 VMD（Enabled）
  ...
  - 缺点：
      ...
      - 可能导致**蓝屏（INACCESSIBLE\_BOOT\_DEVICE）**
...
3.  常见路径：
      ...
      - 微星：Settings → Advanced → PCI Subsystem Settings → VMD
        Configuration
      - 技嘉：Settings → IO Ports → VMD Setup
      - 戴尔/惠普：System Configuration → Storage Options → Enable VMD
        Controller
...
### 2\. 开机蓝屏 INACCESSIBLE\_BOOT\_DEVICE
