...
py-spy 是一款轻量级、非侵入式、低开销的 Python 程序性能分析工具，无需修改代码、无需重启程序、不会影响业务运行，可实时查看
Python 程序的 CPU 耗时、函数调用栈、线程性能，快速定位代码性能瓶颈。
...
1.  **采样分析器 vs 追踪分析器**：py-spy 是采样器，开销极低（通常 \<
    1%），适合在生产环境使用；但它基于统计采样，非常短暂的函数调用可能被遗漏
2.  **GIL 的影响**：py-spy 采样的是实际 CPU 时间，多线程 Python 程序中 GIL 导致的等待不会被计入
3.  **`-n` 参数**：使用 NumPy/OpenCV/OpenVINO 等 C 扩展时，加 `-n` 可以看到 native
    层调用栈，帮助判断瓶颈是否在 C 扩展内部
